...
(function (window, document) {
	...
	window.onscroll = setupMenu;
	window.onresize = setupMenu;
	setupMenu();

	function getMenuLinks () {
		var menuItems = menu.getElementsByTagName('a'),
			menuLinks = new Array(menuItems.length);

		for (var i=0, l=menuItems.length; i<l; i++) {
			menuLinks[i] = [
				menuItems[i],
				document.getElementById( menuItems[i].href.split('#')[1] )
			];
		}
	...
	function setupMenu () {
		var menuHeight   = parseInt( document.defaultView.getComputedStyle(menu, null).height ),
		...
		for (var i=0, l=menuLinks.length; i<l; i++) {
			if (menuLinks[i][0] === closest) {
				menuLinks[i][0].setAttribute('disabled', 'disabled');
			}
			else {
				menuLinks[i][0].removeAttribute('disabled');
			}
		}
	}
})(window, document);
